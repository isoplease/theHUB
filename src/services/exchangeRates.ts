...
function readCachedRate(pair: keyof typeof CACHE_KEYS, allowExpired = false): ExchangeRatePair | null {
...
export async function getExchangeRate(pair: keyof typeof CACHE_KEYS): Promise<ExchangeRatePair> {
  ...
  try {
    ...
    const data: ExchangeRatePair = {
      ...
      rate,
      updatedAt: payload.date ? new Date(`${payload.date}T16:00:00Z`).toISOString() : new Date().toISOString(),
    };

    writeCachedRate(pair, data);
    return data;
  } catch {
    const fallback = readCachedRate(pair, true);
    if (fallback) {
      return fallback;
    }
